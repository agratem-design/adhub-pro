-- ═══════════════════════════════════════════════════════════════════════════
-- Migration: Hardened Idempotency (Unique Constraints & Replay) and Precise Chain Lookup
-- ═══════════════════════════════════════════════════════════════════════════

-- 1. Enforce Database-level Unique Constraints on source_swap_id
DROP INDEX IF EXISTS public.idx_installation_tasks_source_swap_id;
CREATE UNIQUE INDEX IF NOT EXISTS unique_installation_tasks_source_swap_id 
ON public.installation_tasks(source_swap_id) 
WHERE (source_swap_id IS NOT NULL);

DROP INDEX IF EXISTS public.idx_installation_task_items_source_swap_id;
CREATE UNIQUE INDEX IF NOT EXISTS unique_installation_task_items_source_swap_id 
ON public.installation_task_items(source_swap_id) 
WHERE (source_swap_id IS NOT NULL);

-- 2. Enhanced RPC with Complete Idempotent Replay and Predecessor Chain Lookup
...
BEGIN
  ...
  -- ═══════════════════════════════════════════════════════════════════════════
  -- 11. PRECISE PREDECESSOR CHAIN SWAP HANDLING & INDEPENDENT TASK ISSUANCE
  -- ═══════════════════════════════════════════════════════════════════════════
  
  -- A. Precise Predecessor Lookup:
  -- Find the specific predecessor reinstallation task that introduced p_original_billboard_id into this contract
  SELECT it.id INTO v_predecessor_task_id
  FROM public.installation_tasks it
  ...
  WHERE it.contract_id = p_contract_number
    ...
    AND iti.billboard_id = p_original_billboard_id
    AND iti.replacement_status = 'replacement'
    AND it.status IN ('pending', 'in_progress')
    AND it.source_swap_id IS NOT NULL
  ORDER BY it.created_at DESC
  LIMIT 1;

  -- If predecessor reinstallation task is pending/in_progress, supersede/cancel it
  ...
  -- B. Update pending print and cutout items to replacement billboard
  ...
  -- C. Issue ONE NEW INDEPENDENT Reinstallation Task with unique source_swap_id

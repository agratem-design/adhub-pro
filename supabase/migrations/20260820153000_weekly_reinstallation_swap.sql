-- ═══════════════════════════════════════════════════════════════════════════
-- Migration: Enhanced Instant Billboard Swap with Weekly Reinstallation Tasks
-- ═══════════════════════════════════════════════════════════════════════════
-- 
-- Business Invariants:
-- 1. Replacement work type is explicitly "reinstallation" (إعادة تركيب).
-- 2. Original contract installation tasks are preserved for historical record.
-- 3. All replacement billboards swapped in the same operational week (Saturday to Friday)
--    are automatically grouped into ONE open reinstallation task.
-- 4. Reinstallation task numbers are generated from the official central numbering sequence.
-- 5. Zero Financial Delta on Contract: Contract.Total and slot prices are 100% preserved.
-- 6. Zero rows in paused_billboards / paused_billboard_replacements.
-- 7. Chained swaps (A -> B -> C) safely cancel pending uninstalled intermediate items.
...
DECLARE
  ...
  -- Reinstallation task grouping variables
  v_week_start TIMESTAMPTZ;
  v_week_end TIMESTAMPTZ;
  v_reinstall_task_id UUID := NULL;
  v_next_reinstall_num INT := 1;
  v_existing_item_id UUID := NULL;
...
BEGIN
  ...
  -- ═══════════════════════════════════════════════════════════════════════════
  -- 11. REINSTALLATION TASKS (إعادة تركيب) - WEEKLY OPERATIONAL AGGREGATION
  -- ═══════════════════════════════════════════════════════════════════════════
  ...
  -- B. Group into Open Weekly Reinstallation Task (Saturday 00:00:00 to Friday 23:59:59)
  v_week_start := date_trunc('day', CURRENT_TIMESTAMP) - (((EXTRACT(DOW FROM CURRENT_TIMESTAMP)::INT + 1) % 7) || ' days')::INTERVAL;
  v_week_end := v_week_start + INTERVAL '7 days' - INTERVAL '1 second';

  -- Find open weekly reinstallation task
  ...
  -- Create new reinstallation task if no open one exists for this operational week
  ...
  -- C. Add the Replacement Billboard as an Item into the Weekly Reinstallation Task (Idempotency Safe)
  SELECT id INTO v_existing_item_id
  FROM public.installation_task_items
  WHERE task_id = v_reinstall_task_id
    AND billboard_id = p_replacement_billboard_id
    AND notes LIKE '%' || COALESCE(v_orig_bb."Billboard_Name", '#' || p_original_billboard_id) || '%';
  ...
  INSERT INTO public.activity_log (
    ...
    v_contract."Ad Type",
    'تبديل اللوحة #' || p_original_billboard_id || ' باللوحة #' || p_replacement_billboard_id || ' مع جدولة إعادة التركيب الأسبوعية وثبات إجمالي العقد',
    jsonb_build_object(

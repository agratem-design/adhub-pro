-- ═══════════════════════════════════════════════════════════════════════════
-- Migration: Independent Reinstallation Task per Billboard Swap (1 Swap = 1 Task)
-- ═══════════════════════════════════════════════════════════════════════════
-- 
-- Business Invariants:
-- 1. Level 1 (Task Level): ONE billboard swap = ONE independent "reinstallation" task (إعادة تركيب)
--    with its own official sequential task_number from the central sequence.
-- 2. Level 2 (Grouping Level): Weekly grouping in UI/queries groups independent tasks by issuance week (created_at).
-- 3. No shared multi-billboard tasks across different swaps.
-- 4. Chained swaps (A -> B -> C): If task for B was pending, it is marked cancelled/superseded, and a new task is issued for C.
-- 5. Financial Invariant: Contract.Total and slot prices are 100% preserved (0.00 LYD delta).
...
DECLARE
  ...
  -- Reinstallation task variables (1 Swap = 1 Task)
...
BEGIN
  ...
  -- ═══════════════════════════════════════════════════════════════════════════
  -- 11. INDEPENDENT REINSTALLATION TASK PER SWAP (1 Swap = 1 Independent Task)
  -- ═══════════════════════════════════════════════════════════════════════════
  ...
  -- B. Check Idempotency: Does an active reinstallation task already exist for this exact swap?
  SELECT iti.task_id INTO v_reinstall_task_id
  FROM public.installation_task_items iti
  JOIN public.installation_tasks it ON it.id = iti.task_id
  WHERE it.contract_id = p_contract_number
    AND it.task_type = 'reinstallation'
    AND it.status IN ('pending', 'in_progress')
    AND iti.billboard_id = p_replacement_billboard_id
    AND iti.notes LIKE '%' || COALESCE(v_orig_bb."Billboard_Name", '#' || p_original_billboard_id) || '%'
  LIMIT 1;
  ...
  IF v_reinstall_task_id IS NULL THEN
    ...
    -- Insert Independent Task into installation_tasks
